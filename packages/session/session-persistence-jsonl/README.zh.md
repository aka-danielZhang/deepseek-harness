...
- **每会话一个活动 writer**：append 和修复只在所属后端实例内协调；当持久日志自该所有者上次写入以来被推进（共享同一根目录的其他后端实例或进程），append 会大声拒绝，而不是静默交错出重复 seq。该校验以批次为粒度，同一批次内竞速的两个写者仍可能交错。初始同 id 发布仍通过 POSIX 无覆盖硬链接或 Windows 无替换 write-through rename 保持冲突安全。
