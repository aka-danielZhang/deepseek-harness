...
- **One live writer per session** — append and repair are coordinated inside the owning backend instance, and an append rejects loudly when the durable log advanced since this owner's last write (another backend instance or process sharing the root) instead of silently interleaving duplicate seqs; the check is batch-granular, so two writers racing inside one batch can still interleave. Initial same-id publication remains collision-safe through the POSIX no-overwrite hard link or Windows write-through rename without replacement.
