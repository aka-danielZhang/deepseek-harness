...
The package occupies the root-scoped `main` key `conversation`, whose wrapper declares the optional-Session `main.conversation` shell. It registers strict Session header/body entries, View list, composer chain and bar, input regions, Hero regions, queue dock, draft persistence, and phase calculation. `ctx.uiSession.provide()` materializes the Conversation and input sources from the same Session binding and supplies `inputActions` as a stable standard prop. While a desktop toolbar is mounted, the session header portals its title cluster and rightbar corner into the toolbar's hosts through the standard `useToolbarHosts` hook and collapses to its View-tabs row (`null` hosts render the original in-place header; a blank Session stays hidden either way).
