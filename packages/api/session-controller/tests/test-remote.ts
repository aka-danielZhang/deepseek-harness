...
export interface TestSessionRemoteDefaults {
  ...
  readonly recallModelEffort?: (provider: string, model: string) => string | undefined
  readonly rememberModelEffort?: (
    provider: string,
    model: string,
...
function installControllers(
  ...
  if (ctx.get('agentDefaultModel') === undefined) {
    const effortMemory = new Map<string, string>()
    const effortKey = (provider: string, model: string) => `${provider}\0${model}`
    ctx.provide('agentDefaultModel', {
      ...
      recallEffort: (provider: string, model: string) => (
        defaults.recallModelEffort?.(provider, model)
        ?? effortMemory.get(effortKey(provider, model))
      ),
      rememberEffort: async (provider: string, model: string, effort: string | undefined) => {
        if (defaults.rememberModelEffort !== undefined) {
          await defaults.rememberModelEffort(provider, model, effort)
          return
        }
        const key = effortKey(provider, model)
        if (effort === undefined) effortMemory.delete(key)
        else effortMemory.set(key, effort)
      },
    } as never)
  }
