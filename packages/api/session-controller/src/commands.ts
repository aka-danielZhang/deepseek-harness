  ...
  async selectModel(request: SessionSelectModelRequest): Promise<SessionSelectModelValue> {
    ...
    return this.agents.serializeImageAdmission(agent, async () => {
      const persistEffortMemory = async (effort: string | undefined): Promise<void> => {
        try {
          await this.ctx.agentDefaultModel.rememberEffort(
            request.provider,
            request.model,
            effort === undefined ? undefined : ReasoningEffortId(effort),
          )
        } catch (error) {
          this.ctx.logger.warn(
            `session-controller: the model switch applies to this Session but its effort memory was not updated: ${String(error)}`,
          )
        }
      }
      try {
        ...
        let requested: string | undefined = request.reasoningEffort
