...
本包在一个本可完成的回合关闭前，给模型最后一次核对未完成 todo 的机会。它向同一回合注入一条带来源归属的提醒，再由模型完成事项、围绕明确延后重写清单，或解释为何工作仍保持开放。它绝不否决结束，也绝不循环。适合使用 `todo_write` 的自主会话；当开放清单绝不能触发额外模型 step 时不要挂载。
