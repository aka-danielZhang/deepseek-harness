...
- **显式代发名单**：`scripts/publish-fork-extra.json` 列出源码 diff 看不见、但 fork 必须代发的包——目前用于修正上游发布产物本身的缺陷（例：`0.1.6-alpha.1` 的 `dsh-client-ui-primitives` tarball 被上游发布管线剥掉了 `diff` 依赖声明，隔离安装直接炸；fork 仓树内 manifest 正确，代发即修复）。名单变更走 fork 仓普通提交；`prepare-runtime.mjs` 的 FORK_MODIFIED 与插件钉版同步跟随。
